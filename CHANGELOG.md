...
| [`hubspot__conversations`](https://fivetran.github.io/dbt_hubspot/#!/model/model.hubspot.hubspot__conversations) | New end model | | | One record per conversation thread, enriched with inbox, channel, channel account details, and aggregated message metrics. Dynamically enabled in Quickstart. Enabled by default in self-managed dbt projects; to disable, set `hubspot_conversation_enabled` to `False`. |
...
### Enabling/Disabling New Models

The following changes allow for disabling newly added transformations in self-managed dbt projects:

- Adds the `hubspot_conversation_enabled` variable to support enabling/disabling `conversation` models. Dynamically configured in Quickstart; `true` by default otherwise. 
