...
[PR #200](https://github.com/fivetran/dbt_hubspot/pull/200), [PR #202](https://github.com/fivetran/dbt_hubspot/pull/202), [PR #203](https://github.com/fivetran/dbt_hubspot/pull/203), and [PR #TBD](https://github.com/fivetran/dbt_hubspot/pull/TBD) include the following updates:
...
| ------------- | ----------- | --- | --- | ----- |
| [`hubspot__daily_contact_history`](https://fivetran.github.io/dbt_hubspot/#!/model/model.hubspot.hubspot__daily_contact_history) | New end model | | | Final model. Each record represents a contact's state on a given day with lifecycle stage, owner, and other selected properties pivoted into columns. |
...
## Feature Update
- Adds `hubspot__daily_contact_history`, a new incremental end model. Each record represents a contact's state on a given day, with `lifecyclestage`, `hubspot_owner_id`, owner name, and owner email available by default. When `hubspot_email_event_enabled` and `hubspot_email_event_sent_enabled` are `true`, the model also includes daily email engagement metrics (sends, deliveries, opens, clicks, bounces, spam reports, and optionally unsubscribes). When `hubspot_sales_enabled`, `hubspot_engagement_enabled`, and `hubspot_engagement_contact_enabled` are `true`, the model also includes daily engagement counts per type (calls, emails, meetings, notes, tasks). Use `hubspot__contact_property_history_columns` to pivot out additional contact properties.
- Adds the `hubspot__contact_property_history_columns` variable to pivot out additional properties as `string` columns in `hubspot__daily_contact_history`.
