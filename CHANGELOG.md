...
| [stg_hubspot__property_option](https://fivetran.github.io/dbt_hubspot/#!/model/model.hubspot.stg_hubspot__property_option) | Primary Key Change | `property_id` + `property_label` | `property_option_name` + `hubspot_object` + `property_label` | The composite primary key is now `property_option_name`, `hubspot_object`, and `property_label` instead of `property_id` and `property_label`. The `property_id` field is marked as deprecated. |
