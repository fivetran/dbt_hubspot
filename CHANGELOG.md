...
[PR #200](https://github.com/fivetran/dbt_hubspot/pull/200), [PR #202](https://github.com/fivetran/dbt_hubspot/pull/202), [PR #203](https://github.com/fivetran/dbt_hubspot/pull/203), and [PR #204](https://github.com/fivetran/dbt_hubspot/pull/204) include the following updates:

## Schema/Data Change
**32 new models • 0 possible breaking changes**
...
| [`hubspot__daily_deal_history`](https://fivetran.github.io/dbt_hubspot/#!/model/model.hubspot.hubspot__daily_deal_history) | New end model | | | Each record represents a deal's state on a given day with pipeline, stage, amount, owner, team, and other selected properties pivoted into columns. Includes daily engagement activity counts when `hubspot_engagement_deal_enabled` is `true`. |
| [`hubspot__daily_company_history`](https://fivetran.github.io/dbt_hubspot/#!/model/model.hubspot.hubspot__daily_company_history) | New end model | | | Each record represents a company's state on a given day with owner, lifecycle stage, and other selected properties pivoted into columns. Includes daily engagement activity counts when `hubspot_engagement_company_enabled` is `true`. |
...
- Adds the `hubspot__company_property_history_columns` variable to pivot out additional properties as `string` columns in `hubspot__daily_company_history`. By default, `hubspot_owner_id` and `lifecyclestage` are tracked. See the [README](https://github.com/fivetran/dbt_hubspot/tree/main?tab=readme-ov-file#daily-contact-ticket-deal-and-company-history) for configuration details.
