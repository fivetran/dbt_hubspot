...
{% if fivetran_utils.enabled_vars(['hubspot_engagement_enabled','hubspot_engagement_deal_enabled']) %}
    ...
    inner join engagement_deals
        on cast(engagements.engagement_id as {{ dbt.type_int() }}) = cast(engagement_deals.engagement_id as {{ dbt.type_int() }} )
    ...
    left join engagement_deal_agg
        on cast(deals_enhanced.deal_id as {{ dbt.type_int() }}) = cast(engagement_deal_agg.deal_id as {{ dbt.type_int() }} )
